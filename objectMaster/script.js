...
const divisbleByThree = pokémon.filter( p => p.id % 3 ===0 );
...
const justNamesWithBigID = pokémon.filter( p => p.id>100).map(p => p.name);
// console.log(justNamesWithBigID);

const onlyTypePoison = pokémon.filter(p => p.types.length===1 && p.types[0]==="poison").map(p=>p.name);
